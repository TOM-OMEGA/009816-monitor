...
# 導入 AI 判斷模組
try:
    from ai_expert import analyze_us_market
    AI_AVAILABLE = True
except ImportError:
    AI_AVAILABLE = False
...
def generate_us_dashboard(dfs):
    ...
    exp12 = gspc_close.ewm(span=12, adjust=False).mean()
    exp26 = gspc_close.ewm(span=26, adjust=False).mean()
    macd = exp12 - exp26
...
def run_us_ai():
    ...
    # 🤖 美股綜合 AI 判斷 + 台股明日預測
    # =====================
    ai_result = {"decision": "震盪", "confidence": 50, "reason": "AI 未啟用"}
    ...
    if AI_AVAILABLE and "^GSPC" in all_indicators:
        try:
            ...
            # 計算整體漲跌
            spx_prev = float(dfs["^GSPC"]['Close'].iloc[-2])
            spx_pct = (spx_info['price'] / spx_prev - 1) * 100
            
            nasdaq_prev = float(dfs["^IXIC"]['Close'].iloc[-2])
            nasdaq_pct = (nasdaq_info['price'] / nasdaq_prev - 1) * 100
            
            tsm_prev = float(dfs["TSM"]['Close'].iloc[-2])
            tsm_pct = (tsm_info['price'] / tsm_prev - 1) * 100
            
            extra_data = {
                "spx": f"{spx_info['price']:.2f} ({spx_pct:+.2f}%, RSI: {spx_info['rsi']:.1f})",
                "nasdaq": f"{nasdaq_info['price']:.2f} ({nasdaq_pct:+.2f}%, RSI: {nasdaq_info.get('rsi', 0):.1f})",
                "sox": "N/A",
                "tsm": f"{tsm_info['price']:.2f} ({tsm_pct:+.2f}%, {tsm_info.get('trend', 'N/A')})",
                "tech": f"S&P {spx_info['trend']}, NASDAQ {nasdaq_info.get('trend', 'N/A')}"
            }
            
            ai_result = analyze_us_market(extra_data, debug=False)
            
        except Exception as e:
            logging.error(f"美股 AI 判斷異常: {e}")
            ai_result = {"decision": "震盪", "confidence": 50, "reason": "AI 分析異常"}
    
    # 加入 AI 判斷結果
    report.append("---")
    report.append(f"## 🤖 AI 市場研判（台股明日預測）")
    report.append(f"🔮 **明日開盤**： **{ai_result['decision']}**")
    report.append(f"💯 **信心指數**： `{ai_result['confidence']}%`")
    report.append(f"💡 **分析建議**： {ai_result['reason']}")
    report.append("---")
    report.append(f"_此分析將用於台股存股與網格交易判斷_")
    report.append("---")
